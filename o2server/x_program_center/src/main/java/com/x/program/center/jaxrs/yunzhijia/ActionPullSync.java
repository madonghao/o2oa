...
public class ActionPullSync extends BaseAction {
    ActionResult<ActionPullSync.Wo> execute(EffectivePerson effectivePerson) throws Exception {
        try (EntityManagerContainer emc = EntityManagerContainerFactory.instance().create()) {
            if (effectivePerson.isNotManager()) {
                throw new ExceptionAccessDenied(effectivePerson);
            }
            ActionResult<ActionPullSync.Wo> result = new ActionResult<>();
            ActionPullSync.Wo wo = null;
            if (Config.yunzhijia().getEnable()) {
                Business business = new Business(emc);
                SyncOrganization o = new SyncOrganization();
                SyncOrganization.PullResult pullResult = o.execute(business);
                wo = XGsonBuilder.convert(pullResult, ActionPullSync.Wo.class);
            }else{
